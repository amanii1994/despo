...
import { View, StyleSheet, Image, Text, TextInput, ScrollView, AsyncStorage, Platform, TouchableOpacity } from 'react-native';
...
export default class Message extends Component {
  ...
  componentDidMount() {
    ...
    if (this._isMounted) {
      AsyncStorage.getItem("user_id").then((info) => {
        if (info) {
          var u_id = info;
          fetch(apiUrl + 'object=user&action=fetchGeneralMessage&user_id=' + u_id)
            .then((response) =>
              response.json())
            .then((responseJson) => {
  ...
  arcMessage(id) {
    AsyncStorage.getItem("user_id").then((info) => {
      if (info) {
        var u_id = info;
        fetch(apiUrl + 'object=user&action=archiveMessage', {
          method: 'POST',
          headers: new Headers({
  ...
  render() {
    return (
      <View style={{ backgroundColor: '#CAC6C5', width: '100%', height: '100%' }}>
        ...
        <ScrollView>
          <View style={{ flexDirection: 'row', justifyContent: 'center', width: 'auto', marginLeft: wp('8%'), marginRight: wp('3%'), marginTop: hp('2%'), marginBottom: hp('2%') }}>
            <View style={{ width: 'auto', borderRadius: 8, padding: 12, backgroundColor: 'black', textAlign: 'left' }}>
            ...
            <View style={{ width: 'auto', borderRadius: 8, padding: 12, textAlign: 'right' }}>
              <TouchableOpacity style={{ width: wp('43%') }}
